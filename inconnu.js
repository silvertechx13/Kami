...
function inconnuboyLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [SILVER TECH] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = ['1203634@newsletter'];
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
    ...
    await groupEvents(conn, update);
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        ...
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© KAMRAN MD' } }
                    }},
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'KAMRAN MD is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 KAMRAN MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
